...
presets:                # optional, always shown at the top of the music list's Favourites
  - name: Chill
...
### Alexa Music List card

A separate card to put next to the player:

```yaml
type: custom:alexa-music-list-card
title: Music list
default_tab: favorites   # or recent
presets:                 # optional
  - name: Chill
    query: chill playlist
```

**Recent** fills itself with songs that played on your speakers and your searches. **Favourites** shows presets plus anything you star (☆ on the player or in Recent). Tap an item to play it on the speaker selected in the player card, which must be on the same dashboard. The list is saved in your Home Assistant user profile, so every dashboard and device you sign in on shares it.
